...
struct Node *rotateRight(struct Node *y)
{
    ...
    y->height = maxx(getHeight(y->left), getHeight(y->right)) + 1;
    x->height = maxx(getHeight(x->left), getHeight(x->right)) + 1;
...
struct Node *insert(struct Node *root, int value)
{
    ...
    root->height = 1 + maxx(getHeight(root->left), getHeight(root->right));
